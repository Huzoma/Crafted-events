...
import { Ticket, User, Mail, Phone, Loader2, ArrowLeft, Globe } from "lucide-react";
...
export default function VirtualRegistration() {
  ...
  async function handleSubmit(e) {
    ...
    submitData.append("ticketType", "VIRTUAL"); // Explicitly setting to VIRTUAL

    const result = await registerAttendee(submitData);

    if (result.success) {
      // Route to success page to display their digital ticket
    ...
    <main className="min-h-screen grid lg:grid-cols-2 bg-[#09090B]">
      ...
      <div className="flex items-center justify-center p-6 relative">
        ...
        <div className="w-full max-w-md bg-white/5 border border-white/10 rounded-[2rem] p-8 backdrop-blur-xl shadow-2xl relative z-10 overflow-hidden mt-12 lg:mt-0">
          ...
          {/* Decorative background glow (Swapped to purple for Virtual) */}
          <div className="absolute top-[-20%] left-[-20%] w-[60%] h-[60%] bg-purple-600/10 blur-[50px] rounded-full pointer-events-none" />
